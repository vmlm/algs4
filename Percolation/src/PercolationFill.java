...
public class PercolationFill {
    ...
    */
    public boolean isFull(int i, int j) {
        checkLatticeBounds(i, j);
        return (this.lattice[getIndex(i, j)] & 2) == 2;
    }
    ...
    */
    public void open(int i, int j) {
        ...
        if (!isOpen(i, j)) {
            ...
            if (i == 1) {
                uf.union(index+1, 0);
                fill(i, j, index, From.ABOVE); // fill from above
                filled = true;
            }
          ...
          // check below
            if ((i < this.n) && isOpen(i+1, j)) {
                uf.union(index+1, getIndex(i+1, j)+1);
                if (!filled && (isFull(i+1, j))) {
                    fill(i, j, index, From.BELOW); // fill from below
                    filled = true;
                }
            }
            ...
            if ((j > 1) && isOpen(i, j-1)) {
                ...
                if (!filled && (isFull(i, j-1))) {
                    fill(i, j, index, From.LEFT); // fill from below
                    filled = true;
                }
            }
            ...
            if ((j < this.n) && isOpen(i, j+1)) {
                uf.union(index+1, getIndex(i, j+1)+1);
                if (!filled && (isFull(i, j+1))) {
                    fill(i, j, index, From.RIGHT); // fill from below
                    filled = true;
                }
            }
        }
    ...
    /**Fills an open site and all connected sites if adjacent to a filled site. 
     * @param i: Vertical index.
     * @param j: Horizontal index
     * @param index
     * @param direction
     */
    private void fill(int i, int j, int index, From direction) {
        this.lattice[index] = 3;
        int auxindex;

        // check above
        auxindex = getIndex(i-1, j);
        if ((direction != From.ABOVE) && (i > 1) && 
                ((this.lattice[auxindex] & 3) == 1)) {
            fill(i-1, j, auxindex, From.BELOW);
        }

        // check below
        auxindex = getIndex(i+1, j);
        if ((direction != From.BELOW) && (i < this.n) && 
                ((this.lattice[auxindex] & 3) == 1)) {
            fill(i+1, j, auxindex, From.ABOVE);
        }

        // check left
        auxindex = getIndex(i, j-1);
        if ((direction != From.LEFT) && (j > 1) && 
                ((this.lattice[auxindex] & 3) == 1)) {
            fill(i, j-1, auxindex, From.RIGHT);
        }

        // check right
        auxindex = getIndex(i, j+1);
        if ((direction != From.RIGHT) && (j < this.n) && 
                ((this.lattice[auxindex] & 3) == 1)) {
            fill(i, j+1, auxindex, From.LEFT);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int i, j;
        int n = StdIn.readInt();
        PercolationFill p = new PercolationFill(n);
        p.printGrid();
        while (!StdIn.isEmpty()) {
            i = StdIn.readInt();
            j = StdIn.readInt();
